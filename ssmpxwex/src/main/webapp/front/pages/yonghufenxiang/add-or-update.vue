<template>
	<view class="content">
		<form class="app-update-pv">
				...
				<picker @change="fenxiangleixingChange" :value="fenxiangleixingIndex" :range="fenxiangleixingOptions">
					<view :style='{"boxShadow":"0 0 4rpx rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"rgba(0,0,0,.6)","borderRadius":"16rpx","color":"rgba(23, 152, 242, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx"}' class="uni-input">{{ruleForm.fenxiangleixing?ruleForm.fenxiangleixing:"请选择分享类型"}}</view>
				</picker>
			</view>
																					<view :style='{"boxShadow":"0 0 4rpx rgba(0,0,0,.3)","backgroundColor":"rgba(255, 255, 255, 0.25)","borderColor":"#ccc","margin":"0 0 20rpx 0","borderWidth":"0","borderStyle":"solid","height":"108rpx"}' class="cu-form-group" @tap="fenxiangtupianTap" :class='left == "left"?"":"active"'>
				<view :style='{"width":"170rpx","fontSize":"28rpx","color":"rgba(0, 0, 0, 1)","textAlign":"left"}' class="title">分享图片</view>
				<view class="right-input" :style='{textAlign:"left"}' style="padding:0">
					<image :style='{"width":"88rpx","boxShadow":"0 0 16rpx rgba(0,0,0,.3)","borderRadius":"100%","textAlign":"left","height":"88rpx"}' class="avator" v-if="ruleForm.fenxiangtupian" :src="ruleForm.fenxiangtupian" mode="aspectFill"></image>
					<image :style='{"width":"88rpx","boxShadow":"0 0 16rpx rgba(0,0,0,.3)","borderRadius":"100%","textAlign":"left","height":"88rpx"}' class="avator" v-else src="../../static/gen/upload.png" mode="aspectFill"></image>
												...
												<view :style='{"boxShadow":"0 0 16rpx rgba(0,0,0,.3)","backgroundColor":"rgba(255, 255, 255, 0.25)","borderColor":"#ccc","margin":"0 0 20rpx 0","borderWidth":"0","borderStyle":"solid","height":"108rpx"}' class="cu-form-group select">
				<view :style='{"width":"170rpx","fontSize":"28rpx","color":"rgba(0, 0, 0, 1)","textAlign":"left"}' class="title">分享日期</view>
				<picker mode="date" :value="ruleForm.fenxiangriqi" @change="fenxiangriqiChange">
					<view :style='{"boxShadow":"0 0 4rpx rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"rgba(0,0,0,.6)","borderRadius":"16rpx","color":"rgba(23, 152, 242, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx"}' class="uni-input">{{ruleForm.fenxiangriqi?ruleForm.fenxiangriqi:"请选择分享日期"}}</view>
				</picker>
			</view>
												<view :style='{"boxShadow":"0 0 16rpx rgba(0,0,0,.3)","backgroundColor":"rgba(255, 255, 255, 0.25)","borderColor":"#ccc","margin":"0 0 20rpx 0","borderWidth":"0","borderStyle":"solid","height":"108rpx"}' class="cu-form-group">
			...
			<!-- 否 -->
				...
				<view :style='{"width":"170rpx","fontSize":"28rpx","color":"rgba(0, 0, 0, 1)","textAlign":"left"}' class="title">分享内容</view>
				<textarea :style='{"boxShadow":"0 0 4rpx rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"rgba(0,0,0,.6)","borderRadius":"16rpx","color":"rgba(23, 152, 242, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"280rpx"}' v-model="ruleForm.fenxiangneirong" placeholder="分享内容"></textarea>
			</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			this.user = res.data;
						...
						// ss读取
												...
												this.ruleForm.xingming = this.user.xingming
																		...
																		// 下拉框
			res = await this.$api.option(`fenxiangleixing`,`fenxiangleixing`,{});
			this.fenxiangleixingOptions = res.data;
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`yonghufenxiang`, this.ruleForm.id);
		...
		methods: {
			...
			// 多级联动参数
																																	...
																																	fenxiangriqiChange(e) {
				this.ruleForm.fenxiangriqi = e.target.value;
				this.$forceUpdate();
			},
			...
			clicktimeConfirm(val) {
				...
				this.$forceUpdate();
															...
															// 下拉变化
			fenxiangleixingChange(e) {
				this.fenxiangleixingIndex = e.target.value
				this.ruleForm.fenxiangleixing = this.fenxiangleixingOptions[this.fenxiangleixingIndex]
			},
																																																																		
																																				fenxiangtupianTap() {
				let _this = this;
				this.$api.upload(function(res) {
					_this.ruleForm.fenxiangtupian = _this.$base.url + 'upload/' + res.file;
					_this.$forceUpdate();
					_this.$nextTick(()=>{
						_this.styleChange()
					})
			...
			async onSubmitTap() {
																																																																																																																																																																																																																																																																																																																																																																																																																																																																												if((!this.ruleForm.fenxiangbiaoti)){
					this.$utils.msg(`分享标题不能为空`);
					return
				}
																																												if((!this.ruleForm.fenxiangleixing)){
					this.$utils.msg(`分享类型不能为空`);
					return
				}
					...
					return
																																								...
																																								if(this.ruleForm.id){
					await this.$api.update(`yonghufenxiang`, this.ruleForm);
				}else{
					await this.$api.add(`yonghufenxiang`, this.ruleForm);
				}
