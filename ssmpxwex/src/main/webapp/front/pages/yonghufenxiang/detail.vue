	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback" :bottom="100">
		<view class="container">
				...
				<swiper-item v-for="(swiper,index) in swiperList" :key="index">
					...
					<view class="lable" :style='{"width":"auto","fontSize":"28rpx","color":"rgba(6, 6, 6, 1)","textAlign":"left"}'>分享类型：</view>
					<view class="text" :style='{"color":"rgba(0, 0, 0, 1)","textAlign":"left","fontSize":"28rpx"}'>{{detail.fenxiangleixing}}</view>
				</view>
                				                								                								                				<view class="detail-list-item" :style='{"boxShadow":"0 0 0px rgba(0,0,0,.3)","margin":"0 0 2rpx 0","borderColor":"rgba(171, 174, 176, 1)","borderRadius":"0","borderWidth":"2rpx","borderStyle":"none none solid none","height":"68rpx"}'>
					<view class="lable" :style='{"width":"auto","fontSize":"28rpx","color":"rgba(6, 6, 6, 1)","textAlign":"left"}'>分享日期：</view>
					<view class="text" :style='{"color":"rgba(0, 0, 0, 1)","textAlign":"left","fontSize":"28rpx"}'>{{detail.fenxiangriqi}}</view>
				</view>
                				                				<view class="detail-list-item" :style='{"boxShadow":"0 0 0px rgba(0,0,0,.3)","margin":"0 0 2rpx 0","borderColor":"rgba(171, 174, 176, 1)","borderRadius":"0","borderWidth":"2rpx","borderStyle":"none none solid none","height":"68rpx"}'>
				...
				<view class="content" :style='{"color":"#333","fontSize":"28rpx"}'>
					<rich-text style="font-size: 33upx;line-height: 50upx;letter-spacing: 5upx;" :nodes="detail.fenxiangneirong"></rich-text>
				</view>
			</view>
...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','yonghufenxiang');
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.add('storeup', {
								userid: _this.user.id,
								name: _this.detail.fenxiangbiaoti,
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','yonghufenxiang');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('yonghufenxiang', this.id);
				this.detail = res.data;
																												//修改富文本的图片样式
				this.detail.fenxiangneirong = this.detail.fenxiangneirong.replace(/img src/gi,"img style=\"width:100%;\" src");
																// 轮播图片
				this.swiperList = this.detail.fenxiangtupian ? this.detail.fenxiangtupian.split(",") : [];
																																																																											},
			...
			async upCallback(mescroll) {
				                let res = await this.$api.list('discussyonghufenxiang', {
					page: mescroll.num,
					limit: mescroll.size,
					refid: this.id
				});
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussyonghufenxiang/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('yonghufenxiang', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('yonghufenxiang', this.detail);
				this.$utils.msg('审核成功');
