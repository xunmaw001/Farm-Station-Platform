	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
        ...
        <view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			...
			<view v-if="queryIndex==0" class="search-form round">
				...
				<input v-model="searchForm.fenxiangbiaoti" type="text" placeholder="分享标题" ></input>
			</view>
		...
		<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"#efefef"}'>
			<view @tap="onDetailTap(product)" class="uni-product" :style='{"borderRadius":"8rpx","backgroundColor":"rgba(255, 255, 255, 1)"}' v-for="(product,index) in list" :key="index">
								                                <view class="uni-product-title" :style='{"fontSize":"28rpx","lineHeight":"56rpx","color":"#333","textAlign":"left"}'>{{product.fenxiangbiaoti}}</view>
								                				                				                                <view class="image-view">
                    <image :style='{"borderRadius":"8rpx","width":"100%","height":"100%"}' mode="aspectFill" class="uni-product-image" :src="product.fenxiangtupian?product.fenxiangtupian.split(',')[0]:''"></image>
                </view>
                				                				                				                				                				                				                				                				                												<view style="display: flex;justify-content: space-between;">
					<text v-if="isAuth('yonghufenxiang','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
					<text v-if="isAuth('yonghufenxiang','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
				</view>
			</view>
		</view>
		
		
		<button :style='{"boxShadow":"0 0 16rpx rgba(0,0,0,0) inset","backgroundColor":"#409EFF","borderColor":"#409EFF","borderRadius":"8rpx","color":"#fff","borderWidth":"2rpx","width":"160rpx","fontSize":"28rpx","borderStyle":"solid","height":"80rpx"}' v-if="isAuth('yonghufenxiang','新增')" class="add-btn" @click="onAddTap()">新增</button>
	...
	export default {
		...
		methods: {
			...
			queryChange(e) {
				...
				this.searchForm.fenxiangbiaoti="";
			...
			async upCallback(mescroll) {
				...
				let res = await this.$api.list(`yonghufenxiang`, params);
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('yonghufenxiang', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
				...
				let searchForm = {
					...
					limit: this.mescroll.size
												...
												if(this.searchForm.fenxiangbiaoti){
					searchForm['fenxiangbiaoti'] = '%' + this.searchForm.fenxiangbiaoti + '%'
				}	
																if(this.searchForm.fenxiangleixing){
					searchForm['fenxiangleixing'] = '%' + this.searchForm.fenxiangleixing + '%'
				}	
																																																																																												let res = await this.$api.list(`yonghufenxiang`, searchForm);
				// 如果是第一页数据置空
